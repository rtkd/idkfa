...
const Gematria = (() =>
{
     ...
     * @type       {array}
     */
    const graphLengths = [...new Set(Util.flatten(table.map((item) => item[2].map((item) => item.length))).sort())];
     ...
     */
    const toPrime = strTable => x => parseInt(lookup(strTable, (item) => item[0] === x || item[2].includes(Util.isString(x) ? x.toUpperCase() : x) ? item[1] : null));
     ...
     */
    const toLatin = strTable => x => lookup(strTable, (item) => item[0] === x || item[1] === x ? item[2] : null);
