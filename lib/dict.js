...
const Dictionary = (() =>
{
    ...
    const normalize = strWord =>
    {
        ...
        let arrNormalized = arrGraphs.map(graph => Gematria.lookup([], (item) => item[2].includes(graph.toUpperCase()) ? item[2][0] : null));
    ...
    const init = (isNormalized = true) =>
    {
        ...
        arrDict.sort().forEach((word) => Dict.set(word, true));
    };
